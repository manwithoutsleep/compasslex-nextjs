...
**Status:** Ready for Approval (WSL Environment)
...
**Development Environment**: WSL (Windows Subsystem for Linux). All shell commands in this specification use standard Unix/bash syntax, and npm commands work natively without any wrapper.
...
**Command Shell**: All commands in this spec use standard Unix/bash syntax
...
3. **Copy assets between repos** (if needed):
    ...
    ```bash
    # Copy data files
    cp ../compasslex.com/src/assets/data/* public/data/

    # Copy static assets recursively
    cp -r ../compasslex.com/src/assets/* public/assets/
    ```

**Important**: Git and npm commands work natively in WSL. Git commits happen in the **compasslex-nextjs** repository only.
...
```bash
ls -la public/data          # Should show counselor.json and newsletter.json
ls -la public/assets        # Should show newsletters/, pdf/, site-images/, etc.
...
1. **Development Environment**: WSL (all shell commands use Unix/bash syntax)
...
1. **Development Environment**: ✅ RESOLVED - WSL environment. All shell commands in this specification use standard Unix/bash syntax (e.g., `cp -r` for recursive copy, `mkdir -p` for directory creation). Git and npm/npx commands work natively.
